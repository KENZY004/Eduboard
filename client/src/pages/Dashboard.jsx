        ...
        <div className="min-h-screen p-4 sm:p-6 md:p-8 flex flex-col max-w-7xl mx-auto">
            ...
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 sm:mb-12 border-b border-slate-200 dark:border-white/5 pb-4 sm:pb-6 gap-4 sm:gap-0">
                <div>
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight flex items-center gap-2 sm:gap-3">
                        <BsLightningChargeFill className="text-indigo-500 text-xl sm:text-2xl" /> EduBoard <span className="text-xs px-2 py-1 bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded border border-indigo-500/20 font-mono font-normal tracking-wide">PRO</span>
                    </h1>
                    <p className="text-slate-600 dark:text-slate-400 font-light text-sm sm:text-base">Collaborative workspace & infinite canvas</p>
                </div>
                <div className="flex items-center gap-3 sm:gap-6 self-end sm:self-auto">
                    ...
                    <button
                        ...
                        className="p-2 sm:p-3 rounded-full hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors border border-transparent hover:border-slate-200 dark:hover:border-white/10"
                    >
                        <FaSignOutAlt />
                    </button>
                </div>
            ...
            >
                ...
                {isTeacher && (
                    ...
                    >
                        <div className="absolute top-0 right-0 w-[300px] sm:w-[400px] h-[300px] sm:h-[400px] bg-indigo-500/10 dark:bg-indigo-500/20 blur-[120px] rounded-full pointer-events-none -translate-y-1/2 translate-x-1/2 group-hover:bg-indigo-500/20 dark:group-hover:bg-indigo-500/30 transition-all duration-700"></div>

                        <div className="relative z-10">
                            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-slate-100 dark:bg-white/5 rounded-xl sm:rounded-2xl flex items-center justify-center border border-slate-200 dark:border-white/10 text-indigo-500 dark:text-indigo-400 mb-4 sm:mb-6 group-hover:bg-indigo-500 group-hover:text-white transition-all duration-300 shadow-sm">
                                <FaPlus className="text-xl sm:text-2xl" />
                            </div>
                            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 dark:text-white mb-3 sm:mb-4 tracking-tight">Create New <br className="hidden sm:block" /> Whiteboard</h2>
                            <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-base lg:text-lg max-w-md font-light">
                                Start a new session on an infinite high-performance canvas. Optimized for teaching and sketching.
                            </p>
                        </div>

                        <div className="flex items-center gap-2 sm:gap-3 text-indigo-600 dark:text-indigo-400 font-medium mt-6 sm:mt-8 group-hover:translate-x-2 transition-transform text-sm sm:text-base">
                ...
                {!isTeacher && (
                    <div className="surface-card rounded-2xl sm:rounded-3xl p-6 sm:p-8 mb-4 sm:mb-6">
                        <div className="flex items-center gap-3 mb-3 sm:mb-4">
                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-cyan-500/10 dark:bg-cyan-500/20 border border-cyan-500/20 dark:border-cyan-500/30 flex items-center justify-center">
                                <FaSignInAlt className="text-cyan-600 dark:text-cyan-400 text-lg sm:text-xl" />
                            </div>
                            <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Student Access</h3>
                        </div>
                ...
                {!isTeacher && (
                    <div className="surface-card rounded-2xl sm:rounded-3xl p-6 sm:p-8 flex flex-col justify-center relative overflow-hidden">
                        ...
                        <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2">
                            <FaSignInAlt className="text-cyan-600 dark:text-cyan-400" /> Join Session
                        </h3>
                        ...
                        <form onSubmit={joinMeeting} className="flex gap-2">
                            <input
                                ...
                                className="bg-slate-50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-lg px-3 sm:px-4 py-2.5 sm:py-3 text-slate-900 dark:text-white w-full focus:outline-none focus:border-cyan-500/50 transition-colors font-mono text-xs sm:text-sm"
                            />
                            <button className="bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-700 dark:text-white p-2.5 sm:p-3 rounded-lg border border-slate-200 dark:border-white/10 transition-all font-bold">
                                →
                            </button>
                        </form>
                    </div>
                    ...
                    <div className="flex justify-between items-start mb-3 sm:mb-4">
                        <div>
                            <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white mb-1 flex items-center gap-2">
                                <FaFolder className="text-indigo-500 dark:text-indigo-400 text-base sm:text-lg" /> Saved Boards
                            </h3>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-32 text-center">
                                <FaFolder className="text-slate-300 dark:text-slate-600 text-3xl mb-2" />
                                <p className="text-slate-600 dark:text-slate-500 text-sm">No saved boards yet</p>
                            ...
                            savedBoards.map((board) => (
                                <motion.div
                                    ...
                                    className="bg-slate-50 hover:bg-slate-100 dark:bg-white/5 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 rounded-xl p-4 cursor-pointer transition-all group"
                                >
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-slate-900 dark:text-white font-medium text-sm truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                                {isTeacher
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                className="p-2 rounded-lg hover:bg-red-500/10 dark:hover:bg-red-500/20 text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-all opacity-0 group-hover:opacity-100"
